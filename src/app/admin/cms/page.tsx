...
export default function CMSPage() {
  ...
  const [faqEditQuestions, setFaqEditQuestions] = useState<{ id: number; question: string; answer: string }[]>([]);

  const openFaqEdit = (section: FAQSection) => {
    setEditingFaq(section);
    setFaqEditTitle(section.title);
    setFaqEditQuestions(section.questions || []);
  ...
  const handleSaveFaq = (e: React.FormEvent) => {
    ...
    setFaqSections((prev) =>
      prev.map((s) =>
        s.id === editingFaq.id
          ? { ...s, title: faqEditTitle.trim(), questions: faqEditQuestions }
          : s
      )
    );
  ...
  const addFaqQuestion = () => {
    setFaqEditQuestions((prev) => [
      ...prev,
      { id: Date.now(), question: "", answer: "" },
    ]);
  };

  const removeFaqQuestion = (id: number) => {
    setFaqEditQuestions((prev) => prev.filter((q) => q.id !== id));
  };

  const updateFaqQuestion = (id: number, field: "question" | "answer", value: string) => {
    setFaqEditQuestions((prev) =>
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
                ...
                <button
                  onClick={() => {
                    ...
                    if (newFaqTitle) {
                      setFaqSections((prev) => [
                        ...prev,
                        { id: Date.now(), title: newFaqTitle, questions: [] },
                      ]);
                    }
                  }}
                ...
                {faqSections.map((section) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="font-sans text-xs text-[#8C8682] mt-0.5">{(section.questions || []).length} questions</p>
                    </div>
      ...
      {showFaqModal && (
        <div className="fixed inset-0 bg-[#120E0D]/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-xl max-w-lg w-full flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
            {/* Header */}
            <div className="flex items-center justify-between p-6 pb-4 border-b border-gray-100">
            ...
            <form onSubmit={handleSaveFaq} className="flex flex-col flex-1 min-h-0">
              <div className="flex-1 overflow-y-auto p-6 space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder="e.g. Delivery Questions"
                    className="w-full px-4 py-2.5 bg-[#FAF7F2] border border-gray-200 rounded-xl text-sm font-sans focus:outline-none focus:border-[#C9956A] transition-colors"
                  />
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <label className="font-sans text-xs font-bold text-[#1C1512] uppercase tracking-wider">
                      Questions ({faqEditQuestions.length})
                    </label>
                    <button
                      type="button"
                  ...
                  {faqEditQuestions.length === 0 ? (
                    <div className="p-6 text-center border border-dashed border-gray-200 rounded-xl">
                      <p className="font-sans text-xs text-[#8C8682]">No questions yet. Click <span className="font-bold text-[#C9956A]">Add Question</span> to start.</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {faqEditQuestions.map((q, idx) => (
                        <div key={q.id} className="bg-[#FAF7F2] rounded-xl p-4 space-y-3 border border-gray-100">
                          <div className="flex items-center justify-between">
                            <span className="font-sans text-[10px] font-bold text-[#8C8682] uppercase tracking-wider">Question {idx + 1}</span>
                            <button
                              type="button"
                              onClick={() => removeFaqQuestion(q.id)}
                              className="p-1 text-[#8C8682] hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors cursor-pointer"
                            >
                              <Trash2 className="w-3.5 h-3.5" />
                            </button>
                          ...
                          <input
                            ...
                            value={q.question}
                            onChange={(e) => updateFaqQuestion(q.id, "question", e.target.value)}
                            className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm font-sans focus:outline-none focus:border-[#C9956A] transition-colors"
                          />
                          <textarea
                            required
                            placeholder="Answer text..."
                            value={q.answer}
                            onChange={(e) => updateFaqQuestion(q.id, "answer", e.target.value)}
                            rows={3}
                            className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm font-sans focus:outline-none focus:border-[#C9956A] transition-colors resize-none"
                          />
                        </div>
                      ))}
                    </div>
