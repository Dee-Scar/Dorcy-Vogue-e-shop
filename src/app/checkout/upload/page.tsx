    ...
    const loadBankSettings = async () => {
      try {
        const { data } = await supabase
          .from("cms_settings")
          .select("store_settings, contact_whatsapp")
          .eq("id", 1)
          .single();
        ...
        if (cleanWa) {
          setVendorWhatsapp(cleanWa.startsWith("0") ? "234" + cleanWa.slice(1) : cleanWa);
        }
      } catch (err) {
            ...
            >
              ...
              <div className="flex flex-col gap-3 w-full">
                ...
                <button
                  ...
                  className="w-full py-3.5 bg-[#25D366] hover:bg-[#1ebe5d] text-white font-sans text-xs sm:text-sm font-semibold rounded-xl shadow transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer"
                >
